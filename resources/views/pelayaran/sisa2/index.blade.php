    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card mb-4">
                <div class="card-body">
                    @if (!$isReadOnly)
                        <form method="GET" action="{{ route('pelayaran.sisa2.index') }}" class="row g-2 align-items-end">
                            <div class="col-md-8">
                    ...
                    <div class="js-trip-pane {{ $activeTab === $tabKey ? '' : 'd-none' }}"
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <form
                                    action="{{ $meta['key'] === 'pancingan_pribadi' ? route('pelayaran.sisa2.tangkapan-pribadi.store') : route('pelayaran.sisa.tangkapan.store') }}"
                                    method="POST">
                                    @csrf
                                    ...
                                    <fieldset {{ $isReadOnly ? 'disabled' : '' }}>
                                        @if ($meta['key'] === 'pancingan_pribadi')
                                            @php
                                                $defaultFishermen = old('fishermen', $existingPersonalFishermen ?? []);
                                                $defaultFishermen = is_array($defaultFishermen)
                                                    ? array_values($defaultFishermen)
                                                    : [];
                                                if (empty($defaultFishermen)) {
                                                    $defaultFishermen = [''];
                                                }

                                                $defaultPersonalPrices = old('personal_prices', $existingPersonalPrices ?? []);
                                                $defaultPersonalPrices = is_array($defaultPersonalPrices)
                                            ...
                                            @if ($errors->has('anglers') || $errors->has('fishermen'))
                                                <div class="alert alert-danger py-2">
                                                    {{ $errors->first('anglers') ?: $errors->first('fishermen') }}
                                                </div>
                                            @endif

                                            <div class="card border mb-3">
                                                <div class="card-body">
                                                    <h6 class="mb-2">1. Definisikan Harga per Master Ikan Tangkapan</h6>
                                                    <div class="table-responsive">
                                                        <table class="table table-bordered table-sm mb-0">
                                                            <thead>
                                                                <tr>
                                                                    <th>Master Ikan Tangkapan</th>
                                                                    <th class="required-asterisk">Harga per Kg (Rp)</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                            ...
                                            <div class="card border mb-3">
                                                <div class="card-body">
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        <h6 class="mb-0">2. Input Qty per Nelayan</h6>
                                                        <div class="d-flex gap-2">
                                                            <button type="button"
                                                                class="btn btn-sm btn-outline-secondary js-add-fisherman-column">
                                                                Tambah Kolom Nelayan
                                                            </button>
                                                            <button type="button"
                                                                class="btn btn-sm btn-outline-danger js-remove-fisherman-column">
                                                                    ...
                                                                    <tr
                                                                        ...
                                                                        @foreach ($defaultFishermen as $fisherIndex => $fisherName)
                                                                            <td class="js-fisherman-cell"
                                                                                data-fisher-index="{{ $fisherIndex }}">
                                                                                <input type="number"
                                                                                    class="form-control js-personal-weight-input"
                                                                                    min="0" step="0.01"
                                                                                    name="personal_weights[{{ $fisherIndex }}][{{ $ikanTangkapan->id_ikan_tangkapan }}]"
                                                                                    value="{{ old('personal_weights.' . $fisherIndex . '.' . $ikanTangkapan->id_ikan_tangkapan, $defaultPersonalWeights[$fisherIndex][$ikanTangkapan->id_ikan_tangkapan] ?? '') }}"
                                                                                    placeholder="0">
                                                                            </td>
                                                                        @endforeach
                                                                    </tr>
    ...
    @if ($selectedPelayaran)
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                var refreshPersonalMatrixIndexes = function(table) {
                    if (!table) {
                        return;
                    }

                    var headers = Array.from(table.querySelectorAll('.js-fisherman-header'));
                    headers.forEach(function(header, fisherIndex) {
                        header.setAttribute('data-fisher-index', String(fisherIndex));
                        var nameInput = header.querySelector('input');
                        if (nameInput) {
                            nameInput.name = 'fishermen[' + fisherIndex + ']';
                        }
                    });

                    table.querySelectorAll('tbody tr[data-ikan-id]').forEach(function(row) {
                        var ikanId = row.getAttribute('data-ikan-id');
                        var cells = Array.from(row.querySelectorAll('.js-fisherman-cell'));

                        cells.forEach(function(cell, fisherIndex) {
                            cell.setAttribute('data-fisher-index', String(fisherIndex));
                            var beratInput = cell.querySelector('input');
                            if (beratInput) {
                                beratInput.name = 'personal_weights[' + fisherIndex + '][' + ikanId + ']';
                            }
                        });
                    });
                };
                ...
                var addPersonalFishermanColumn = function(table) {
                    ...
                    var nextIndex = headRow.querySelectorAll('.js-fisherman-header').length;
                    headRow.insertAdjacentHTML('beforeend',
                        '<th class="js-fisherman-header" data-fisher-index="' + nextIndex +
                        '" style="min-width: 220px;"><input type="text" class="form-control" name="fishermen[' +
                        nextIndex + ']" value="" placeholder="Nama nelayan" maxlength="120"></th>'
                    );

                    table.querySelectorAll('tbody tr[data-ikan-id]').forEach(function(row) {
                        var ikanId = row.getAttribute('data-ikan-id');
                        row.insertAdjacentHTML('beforeend',
                            '<td class="js-fisherman-cell" data-fisher-index="' + nextIndex +
                            '"><input type="number" class="form-control js-personal-weight-input" min="0" step="0.01" name="personal_weights[' +
                            nextIndex + '][' + ikanId + ']" value="" placeholder="0"></td>'
                        );
                    });

                    refreshPersonalMatrixIndexes(table);
                };

                var removePersonalFishermanColumn = function(table) {
                    if (!table) {
                        return;
                    }
                    ...
                    var fisherIndex = lastHeader.getAttribute('data-fisher-index');
                    lastHeader.remove();

                    table.querySelectorAll('tbody tr[data-ikan-id]').forEach(function(row) {
                        var cell = row.querySelector('.js-fisherman-cell[data-fisher-index="' + fisherIndex + '"]');
                        if (cell) {
                            cell.remove();
                        }
                    });
                ...
                document.querySelectorAll('.js-personal-matrix-table').forEach(function(table) {
                    refreshPersonalMatrixIndexes(table);
                });
                ...
                document.addEventListener('click', function(event) {
                    ...
                    if (addFishermanColumnBtn) {
                        event.preventDefault();
                        var table = addFishermanColumnBtn.closest('.card-body').querySelector(
                            '.js-personal-matrix-table');
                        addPersonalFishermanColumn(table);
                        return;
                    }

                    var removeFishermanColumnBtn = event.target.closest('.js-remove-fisherman-column');
                    if (removeFishermanColumnBtn) {
                        event.preventDefault();
                        var table = removeFishermanColumnBtn.closest('.card-body').querySelector(
                            '.js-personal-matrix-table');
                        removePersonalFishermanColumn(table);
                        return;
                    }
                ...
                document.querySelectorAll('form').forEach(function(form) {
                    form.addEventListener('submit', function() {
                        ...
                        form.querySelectorAll('input[name*="[harga_"], input[name*="[harga_per_kg"], input[name*="harga_ikan"], input[name*="jumlah["], input[name*="personal_prices["]').forEach(function(input) {
