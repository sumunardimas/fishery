...
<div class="alert d-flex alert-{{ $type }} align-items-start" style="border-radius: 20px" role="alert" id="alert{{$randId}}">
    <div class="flex-nowrap">
        {!! $icon[$type] ?? $icon['danger'] !!}
    </div>
    <div class="ml-2 mr-3 mb-0">
        <strong>{{ $titles[$type] ?? $titles['danger'] }}</strong>
        <p class="mb-0"> {{ $message }}</p>
    </div>
    <button class="btn ml-auto p-0 text-{{ $type }}" onclick="document.getElementById('alert{{$randId}}').remove();">
